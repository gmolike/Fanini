...
const FIRST_NAMES = [
  'Max', 'Anna', 'Tim', 'Sarah', 'Felix', 'Laura', 'Jan', 'Lisa',
  'Tom', 'Julia', 'Ben', 'Emma', 'Paul', 'Mia', 'Leon', 'Sophie',
  'Finn', 'Marie', 'Luis', 'Lena', 'Noah', 'Hannah', 'Elias', 'Lea'
];
...
export const generateMembers = async (count: number): Promise<any[]> => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const member = {
      ...
      telefon: i % 3 === 0 ? `+49 30 ${Math.floor(Math.random() * 9000000 + 1000000)}` : null,
      ist_aktiv: Math.random() > 0.1, // 90% aktiv
      hat_vertraulichkeitserklaerung: Math.random() > 0.2, // 80% haben unterschrieben
      mitglied_seit: dateHelpers.randomMemberSince(),
      beschreibung: i % 4 === 0 ? DESCRIPTIONS[i % DESCRIPTIONS.length] : null,
      sichtbarkeit_email: ['ALLE', 'MITGLIEDER', 'PRIVAT'][i % 3],
      sichtbarkeit_telefon: 'MITGLIEDER',
      sichtbarkeit_profil: ['ALLE', 'MITGLIEDER'][i % 2],
