// apps/api/src/infrastructure/database/seed/seeders/12-seedSocialMediaPosts.ts
...
const POST_TEMPLATES = [
  {
    inhalt:
      "🔥 MATCHDAY! Heute um 15:00 Uhr gegen Union Berlin II! Wir sehen uns im Block! #Spandau1904 #NurDerESV",
    plattform: ["twitter", "instagram"],
    hashtags: ["Spandau1904", "NurDerESV", "Matchday"],
  },
  {
    inhalt:
      "📸 Impressionen vom letzten Heimspiel! Was für eine Stimmung! 💙 Danke an alle die dabei waren!",
    plattform: ["instagram", "facebook"],
    hashtags: ["EintrachtSpandau", "Heimspiel", "Fans"],
  },
  {
    inhalt:
      "🚌 Noch Plätze frei für die Auswärtsfahrt nach Cottbus! Anmeldung über unsere Website.",
    plattform: ["twitter", "instagram", "facebook"],
    hashtags: ["Auswärtsfahrt", "Spandau", "OnTour"],
  },
  {
    inhalt:
      "🎮 Unser LoL-Team hat es in die Playoffs geschafft! GG WP! #SpandauEsports",
    plattform: ["twitter", "instagram"],
    hashtags: ["SpandauEsports", "LeagueOfLegends", "Playoffs"],
  },
  {
    inhalt:
      "📅 Save the Date: Jahreshauptversammlung am 15.03.! Alle Mitglieder sind herzlich eingeladen.",
    plattform: ["facebook"],
    hashtags: ["JHV", "Vereinsleben"],
  },
];

const seedSocialMediaPosts = async (
  ...
  try {
    ...
    for (let i = 0; i < postCount; i++) {
      ...
      const relatedEvent =
        Math.random() > 0.5 ? randomElement(events as any[]) : null;
      ...
      posts.push({
        ...
        status,
        erstellt_von: "mbr_medien", // Korrigiert - Member ID!
        erstellt_am: new Date(),
        approved_von: status === "VEROEFFENTLICHT" ? "mbr_beirat1" : null, // Korrigiert - Member ID!
        approved_am: status === "VEROEFFENTLICHT" ? new Date() : null,
        hashtags: JSON.stringify(template.hashtags),
        medien_urls:
          Math.random() > 0.5
            ? JSON.stringify([
                `https://storage.example.com/social/${generateId("img")}.jpg`,
              ])
            : null,
      });
    }
