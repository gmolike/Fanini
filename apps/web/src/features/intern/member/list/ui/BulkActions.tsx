...
import { type BulkAction, type MemberRole, ROLE_CONFIG } from '@/entities/intern/member';
...
type MemberBulkActionsProps = {
  ...
  onBulkAction: (action: BulkAction, data?: { roleId?: MemberRole; reason?: string }) => void;
...
export const BulkActions = ({
  ...
  const roleOptions = Object.entries(ROLE_CONFIG)
    ...
    .map(([value, config]) => ({
      value,
      label: (config as unknown as { label: string }).label,
    }));
