// frontend/src/widgets/public/organization-structure/ui/Widget.tsx
import { motion } from 'framer-motion';
import { Network } from 'lucide-react';

import {
  type OrganizationNode,
  useOrganizationStructure,
} from '@/entities/public/organization-structure';

import { AnimatedValue, GlassCard, LoadingState, OrgChart } from '@/shared/ui';
import type { OrgChartNode } from '@/shared/ui/charts';

export const OrganizationStructureWidget = () => {
  const structureQuery = useOrganizationStructure();

  // Transform OrganizationNode to OrgChartNode
  const transformToOrgChartNodes = (
    node: OrganizationNode,
    parentId?: string,
    level = 0
  ): OrgChartNode[] => {
    const orgNode: OrgChartNode = {
      id: node.id,
      label: node.name,
      department: node.description ?? '',
      level,
      type: node.type === 'root' || node.type === 'partner' ? 'board' : 'team',
      parentId,
      memberCount: node.memberCount,
    };

    const nodes = [orgNode];

    if (node.children && node.children.length > 0) {
      node.children.forEach((child: OrganizationNode) => {
        nodes.push(...transformToOrgChartNodes(child, node.id, level + 1));
      });
    }

    return nodes;
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-[var(--color-fanini-blue)]/5 via-transparent to-[var(--color-fanini-red)]/5">
      {/* Hero Section */}
      <div className="relative overflow-hidden py-20">
        <div className="absolute inset-0">
          <div className="absolute -top-40 -right-40 h-80 w-80 animate-pulse rounded-full bg-[var(--color-fanini-blue)]/10 blur-3xl" />
          <div className="absolute -bottom-40 -left-40 h-80 w-80 animate-pulse rounded-full bg-[var(--color-fanini-red)]/10 blur-3xl delay-1000" />
        </div>

        <div className="relative z-10 container mx-auto px-4 text-center">
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            transition={{ type: 'spring' }}
            className="mb-6 inline-flex rounded-full bg-gradient-to-r from-[var(--color-fanini-blue)] to-[var(--color-fanini-red)] p-5"
          >
            <Network className="h-12 w-12 text-white" />
          </motion.div>

          <AnimatedValue delay={0.2}>
            <h1 className="mb-4 bg-gradient-to-r from-[var(--color-fanini-blue)] to-[var(--color-fanini-red)] bg-clip-text text-5xl font-bold text-transparent md:text-7xl">
              Vereinsstruktur
            </h1>
          </AnimatedValue>

          <AnimatedValue delay={0.3}>
            <p className="mx-auto max-w-2xl text-xl text-[var(--color-muted-foreground)]">
              Unsere Organisation im Überblick - Teams, Partner und Strukturen
            </p>
          </AnimatedValue>
        </div>
      </div>

      {/* Content */}
      <div className="container mx-auto px-4 pb-20">
        <LoadingState query={structureQuery}>
          {data => {
            const orgChartNodes = transformToOrgChartNodes(data);

            return (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.4 }}
              >
                <GlassCard className="p-8">
                  <OrgChart
                    nodes={orgChartNodes}
                    expandable
                    onNodeClick={_node => {
                      // TODO: Implement node click handler
                    }}
                  />
                </GlassCard>
              </motion.div>
            );
          }}
        </LoadingState>
      </div>
    </div>
  );
};
